...
# GitHub repository owner/name for releases and service file
# Override with: REPO=DevWizardProjects/xray-torrent-blocker-main
REPO="${REPO:-kutovoys/xray-torrent-blocker}"
...
if [ "$INSTALL_FROM_PACKAGE" = false ]; then
	...
	URL="https://github.com/${REPO}/releases/download/${LATEST_RELEASE}/xray-torrent-blocker-${LATEST_RELEASE}-linux-${ARCH}.tar.gz"
	
	curl -sL "$URL" -o tblocker.tar.gz || true
	
	if [ ! -s "tblocker.tar.gz" ]; then
		print_error "Failed to download tblocker from ${REPO}. If this fork has no releases, set REPO=kutovoys/xray-torrent-blocker or install from packages."
		exit 1
	fi
	...
	curl -sL https://raw.githubusercontent.com/${REPO}/main/tblocker.service -o /etc/systemd/system/tblocker.service
fi
...
print_info "https://github.com/${REPO}"
echo "==============================================================="