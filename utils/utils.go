...
func handleLogEntry(line string) {
	...
	if strings.ToLower(config.EnforcementMode) == "firewall" {
		go BlockIP(ip)
	} else {
		go executeBlockCommand(usernameStr, ip, time.Duration(config.BlockDuration)*time.Minute)
	}
	log.Printf("User %s with IP: %s blocked for %d minutes (mode: %s)\n", usernameStr, ip, config.BlockDuration, config.EnforcementMode)
...
func UnblockIPAfterDelay(ip string, delay time.Duration, username string) {
	...
	if strings.ToLower(config.EnforcementMode) == "firewall" {
		...
		executeUnblockCommand(username, ip)
	}
...
// Command-based enforcement helpers
func executeBlockCommand(username string, ip string, duration time.Duration) {
	if strings.TrimSpace(config.OnBlockCommand) == "" {
		log.Printf("OnBlockCommand is empty, skipping command execution")
		return
	}

	cmdStr := replaceCommandPlaceholders(config.OnBlockCommand, username, ip, duration)
	cmd := exec.Command("/bin/sh", "-c", cmdStr)
	if err := cmd.Run(); err != nil {
		log.Printf("Error executing OnBlockCommand: %v", err)
	}
}

func executeUnblockCommand(username string, ip string) {
	if strings.TrimSpace(config.OnUnblockCommand) == "" {
		log.Printf("OnUnblockCommand is empty, skipping command execution")
		return
	}

	cmdStr := replaceCommandPlaceholders(config.OnUnblockCommand, username, ip, 0)
	cmd := exec.Command("/bin/sh", "-c", cmdStr)
	if err := cmd.Run(); err != nil {
		log.Printf("Error executing OnUnblockCommand: %v", err)
	}
}

func replaceCommandPlaceholders(template string, username string, ip string, duration time.Duration) string {
	cleanUsername := processUsernameForWebhook(username)
	replacer := strings.NewReplacer(
		"{username}", cleanUsername,
		"{raw_username}", username,
		"{ip}", ip,
		"{duration_minutes}", fmt.Sprintf("%d", int(duration.Minutes())),
	)
