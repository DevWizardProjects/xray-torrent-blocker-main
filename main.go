...
func initConfig() {
	...
	// Initialize firewall only in firewall enforcement mode
	if config.EnforcementMode == "" || config.EnforcementMode == "firewall" {
		...
		log.Printf("Enforcement mode: %s (firewall manager not initialized)", config.EnforcementMode)
	}
